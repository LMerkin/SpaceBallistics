// vim:ts=2:et
//===========================================================================//
//                   "SpaceBallistics/PhysEffects/GravityFld.h":             //
//===========================================================================//
#pragma  once
...
#include <cmath>
#include <stdexcept>
// TODO: Replace GSL by our own implementation of Legendre Polynomials:
...
namespace SpaceBallistics
{
  ...
  public:
    ...
    )
    {
      ...
      for (int m = 0; m <= a_n; ++m)
      {
        Angle  ml = Angle(double(m) * lambda);
        cosMLambda[m] = Cos(ml);
        sinMLambda[m] = Sin(ml);
      }
      ...
      for (int l = 2; l <= a_n; ++l)
      {
        ...
        for (int  m = 0; m <= maxM; ++m)
        {
          ...
          SpherHCoeffs const& SHC = BodyData<EGB>::GravFldModelCoeffs[jm];
      ...
      for (int i = 0; i < 3; ++i)
      {
        ...
        // as assumed in the "GravFldModelCoeffs", so compensate for that:
        F[i] *= SqRt4Pi;
